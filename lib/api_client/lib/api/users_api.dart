...
class UsersApi {
  ...
  /// Update a user
  ///
  /// Update a user with the provided ID and data
  ///
  /// Note: This method returns the HTTP [Response].
  ///
  /// Parameters:
  ...
  /// * [String] id (required):
  ///   User ID
  /// * [DtosUpdateUserRequest] updateRequest (required):
  ///   User data to update
  Future<Response> usersIdPutWithHttpInfo(String id, DtosUpdateUserRequest updateRequest) async {
    // ignore: prefer_const_declarations
    final path = r'/users/{id}'
      .replaceAll('{id}', id);

    // ignore: prefer_final_locals
    Object postBody = updateRequest;
  ...
  /// * [DtosUpdateUserRequest] updateRequest (required):
  ///   User data to update
  Future<DtosGetUserResponse?> usersIdPut(String id, DtosUpdateUserRequest updateRequest) async {
    final response = await usersIdPutWithHttpInfo(id, updateRequest);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
